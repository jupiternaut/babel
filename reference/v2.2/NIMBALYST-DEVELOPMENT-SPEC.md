# 巴别塔 × Nimbalyst：三栏工作台与任务执行看板开发 SPEC

版本：v2.2 · 日期：2026-09-14 · 状态：待实施的开发规格。

本文件落实用户确认的 UI v2，以及本地 Cursor Agent Kanban 的交互体验。主应用仍是 Nimbalyst。本文不表示 Nimbalyst 已完成改造，也不把独立 Cursor 演示页面算作 Nimbalyst 验收。
...
第一项交付是 **Nimbalyst 内部可鼠标交互、无需模型 API Key 的 UI 原型**：左栏选择、四列任务、右栏详情、模拟执行事件、模拟差异和历史都可实际操作。通过后，再实现 Ubuntu 后台及真实 Pi 链路。
...
默认完成策略为 `verified_and_reviewed`。Agent 给出结果后，所有必需验收项通过才能进入待审；用户接受后 DONE。人工豁免必须单独记录是谁、何时、哪一项、理由，并显示“含人工豁免”；不能把 skipped 标为 passed。`verified_auto` 仅用于用户配置了可机器验证完成规则的任务。
...
```text
Nimbalyst renderer
  BabelWorkbench / BabelSidebar / BabelTaskBoard / TaskDetailPane
                         │ command/query/event
  TrackerDataSource ────┼── DemoTrackerDataSource（M0，隔离的原生条目）
                         └── BabelTrackerDataSource（生产，宿主代理请求）
Nimbalyst privileged host
  Gateway connection + GatewayAgentProtocol + credential references
                         │ authenticated HTTPS + SSE
Ubuntu Babel Gateway
  tasks / runs / review / outbox / events / Google sync / SSH collector
...
拟新增目录：宿主源码中的 `components/BabelWorkbench/`、`store/babel/`，共享逻辑中的 `babel/contracts/`、`babel/adapters/`；真实路径由 NB-00 根据依赖边界确定，不能为这些设计名称编造上游 API。

### 必须扩展的窄接口

工作台采用原 Trackers 的新执行展示模式：复用 TrackerSidebar 的视图/类型树和原条目状态，外围增加设备、项目、集成、运维导航。BabelWorkbench 是布局名称，不是一套独立任务管理器。原生与简洁布局均消费同一份 tracker atoms；具体映射以配套源码审计文档为准。
...
有副作用的命令带 `Idempotency-Key`，任务编辑/启动带 `expectedRevision`。同键同负载重放返回原结果，同键不同负载 409；同一任务至多一个未核实终止的执行。过期版本 409、权限不足 403、字段不合法 422、节点不可用给出可区分的错误 code。响应中只有用户可读说明，不回显密钥或原始 SSH 命令。

事件包最少包含 `eventId/projectId/taskId/runId/type/seq/occurredAt/payload`。按 run 的 seq 去重并排序；全局 cursor 负责断线补发。首次读取快照及后续事件订阅必须共享一致游标，防止两者间隙丢事件；游标过期要求重拉快照。
...
原型运行在 Nimbalyst 的独立开发 profile，不改正式安装和用户数据库。demo 由宿主启动配置明确启用；与生产账号/凭据存储隔离。demo adapter 不读取现有 API Key、OAuth token 或 SSH key，也不调用真实 Agent 协议。
...
| ID | 依赖 | 具体工作 | 可检查交付 |
|---|---|---|---|
| NB-00 | 无 | 固定 Nimbalyst/参考提交，核实许可、dirty 状态、宿主扩展点和构建命令 | baseline.md、版本与接口表、独立 D 盘开发检出和 profile 方案 |
| NB-01 | NB-00 | 编写 v2 task/run/event/API 和新 tasks.json；保留旧合同归档 | 类型、正反例、旧新字段迁移表、契约检查 |
| NB-02 | NB-01 | 三栏 shell、侧栏、主题、响应式、选中卡片状态 | Nimbalyst 内的可启动页面；五卡视觉 fixture |
| NB-03 | NB-02 | DemoTrackerDataSource、新建编辑、卡片操作、模拟事件、隔离持久化 | 无 Key 操作闭环及恢复/重置测试 |
| NB-04 | NB-03 | 会话/差异/历史复用与加载状态；输入草稿保留 | 右栏交互测试、模拟差异产物；M0 演示包 |
| NB-05 | NB-01 | Ubuntu Gateway、认证、SQLite 迁移、幂等、outbox、SSE | 合成数据库与重启/并发/断线故障测试 |
| NB-06 | NB-04, NB-05 | BabelTrackerDataSource、原 Tracker 保存路径分流、拖拽命令 | 两客户端一致性、生产/UI adapter 一致性、本地 Tracker 回归 |
| NB-07 | NB-05 | 单台受管 Pi Worker、隔离执行、消息/取消/证据、执行身份恢复 | 合成 Git 项目真实运行；不重复执行证据 |
| NB-08 | NB-06, NB-07 | 首条真实 TODO→Pi→验收→DONE→ARCHIVED 链路 | 在 Nimbalyst 中完成全过程，run/task ID 对照；M1 |
...
先完成 M0 让用户试 UI，再接真实执行。安装依赖、生成 OAuth 客户端、绑定账号、连接真实设备等实施动作不因本规格文件自动发生。
...
M0 交付源码、独立启动方式、五卡截图、操作手册和 UI 验收记录；M1 增加部署、备份恢复、版本迁移、真实执行证据；M2/M3 分别增加外部服务和各平台证据。

原 spec 包还含 Nextcloud/Deck v1 内容。本文和 ADR-003/004 确定新业务设计；旧机器合同仅是历史基线。NB-01 必须把正文、OpenAPI、事件、任务列表、执行提示词同步为 v2，并让契约检查覆盖本文要求，再接生产后台；不能用文档优先级掩盖互相矛盾的接口。
