...
当前执行顺序见 [开发 SPEC 的 NB 清单](NIMBALYST-DEVELOPMENT-SPEC.md)；必须同时落实 [Trackers 映射的 MAP 子项](NIMBALYST-TRACKER-MAPPING.md)。下面 WB 清单仍为历史基线。

> 2026-09-14：以下与 tasks.json 均为待迁移的旧任务清单。先落实 [ADR-003](decisions/ADR-003.md) 的契约/任务一致修订；不要直接派发旧 Talk/Deck 架构实施。
