...
当前实施请使用 [Nimbalyst 开发提示词](NIMBALYST-IMPLEMENTER-PROMPT.md)，下面全文是历史 v1 参考。

> 2026-09-14：下方提示词已过时，保留供迁移参考，不得直接执行。先读 [ADR-003](decisions/ADR-003.md) 和 [Nimbalyst 系统规格](NIMBALYST-SYSTEM-SPEC.md)，将 SPEC、API/事件契约、tasks.json、TASKS.md 与本提示词一致迁移后，才进入应用实施。底座确定为 Nimbalyst，不能再自行改选 Cline/Vibe Kanban 作为主应用。
