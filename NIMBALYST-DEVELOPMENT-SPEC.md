...
UI/UX 主导当前场景、组件与交付顺序：设计与 Mac 原生运行基线并行，先形成 V01 首页和 V03 运行详情的可操作母版，再补其余状态和对应业务缺口。浅色磨砂为主稿，深色同步适配；视觉、交互与相关业务证据一起验收。具体材质、回退与证据按 [视觉合同](design/visual-contract.md) 执行，旧 v2 不构成新玻璃母版已获批准的证明。

版本：v2.3 · 日期：2026-09-14 · 状态：待实施的开发规格。

本文件落实 UI v2 的结构方向、原生 Tracker 映射，以及新增能力必须具备 GUI/TUI/CLI 等价业务操作和 Hooks 的要求；材质以当前视觉合同为准。Nimbalyst 是桌面宿主；领域核心、终端与自动化入口独立于图形窗口运行。本文不表示已完成改造，也不把独立 Cursor 演示算作 Nimbalyst 验收。旧版完整保留在 [v2.2 存档](reference/v2.2/README.md)。
...
侧栏、顶栏、详情外框与浮层采用 CSS 磨砂半透明、细边缘高光和柔和层次；正文、编辑器、代码、日志与普通任务卡保留稳定可读表面。只为宿主未覆盖的透明度、模糊和边缘新增 Babel 局部材质变量，不创建第二套主题。`simple-liquid-glass` 是小面积导航/浮层的折射候选，须通过与 CSS 基线的原生输入、可读性和性能对照；默认关闭跟随、弹性位移、涟漪，不引入整窗 WebGL 或背景截图链路。

运行中改变减少动态效果设置须立即关闭位移、视差、弹簧与涟漪；减少透明度或渲染能力不足时回退到不透明主题表面，保留所有操作和层级。系统偏好不能可靠映射时提供应用内关闭入口。材质不变形文字、焦点轮廓或点击区域，不改变 Portal/菜单定位和业务命令路由。
...
下表保留历史里程碑与依赖，不表示当前全部待实施。以 `implementation/` 的当前验收记录核对实际缺口；已有共享核心与三端实现继续复用，UI/UX 切片可在现有合同上推进。旧 `tasks.json` 中 WB 编号不直接派发；需要修改公共合同的工作仍先同步机器合同和任务清单。

| ID | 依赖 | 具体工作 | 可检查交付 |
|---|---|---|---|
| NB-00 | 无 | 固定提交、许可、dirty 状态；盘点 Electron 耦合、TUI 技术与无头服务边界 | baseline.md、CAPABILITY-MATRIX、构建命令、Mac 本地检出和独立 profile |
...
| 编号 | 场景 | 通过标准 |
|---|---|---|
| UI-01 | MacBook M3 原生 Electron，1586×992，五张 fixture 卡 | 左导航、中四列、右详情；中央无监控/同步横幅；按当前视觉合同验收，v2 仅为结构参考 |
...
| UI-08 | 浅深主题、激活/失焦、明暗背景的磨砂表面 | 材质层次可见，文字与点击区域不变形；普通文本在实际合成背景上以 4.5:1 为目标，无主题切换闪白 |
| UI-09 | 运行中减少动效/关闭透明度、中文组合输入、Tab/Esc | 即时回退并保留全部操作；输入、文本选择、焦点恢复与浮层定位正常 |
| UI-10 | 同内容的 CSS 磨砂/局部折射对照及重复开关浮层 | 按视觉合同记录帧间隔、长任务、内存、空闲 CPU 与资源释放；未测或未过预算时保留 CSS 基线 |
