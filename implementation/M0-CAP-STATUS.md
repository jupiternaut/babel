...
| **04 标题、正文、字段编辑** · 部分实现 | 原生 `TrackerItemDetail`，适配器 `update-item`/`update-items → task.update`；`update-item-content` 携带草稿起始版本，原生正文复用富文本编辑器显式保存与冲突选择 | `e` 仅提供标题/正文编辑，携带选中记录 revision | `task update --expected-revision` | `babel/tests/edit-revision-closed-loop.test.ts` 验证适配/HTTP/headless 输入及过期版本；本轮 `title-native.spec.ts` 原生中文标题保存、跨端更新与冲突不覆盖已通过。标题使用显式保存及草稿起始 revision。**本片**：正文适配拒绝无版本/未知 JSON 结构，TUI 旧版本失败保留编辑浮层、冻结原目标并阻止重复保存；见 [TASKS M0-04a](../TASKS.md)。**下一步**：独立 Mac/真实 PTY 富文本与冲突验收，补字段编辑及完整选区/断线恢复；完整 CAP-04 仍未通过。 |
