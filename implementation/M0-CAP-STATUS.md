...
## M0-17a 终端输入增量（2026-09-16）

单独 Esc 等待 100ms 无后续字节后退出弹层；在窗口内到达的方向键/F 键/鼠标前缀继续解析，粘贴正文中的 Escape 不触发退出。保留分段粘贴结束符，退出时取消未触发的计时器。Mac 真实 POSIX PTY 检查了字段取消无提交、分段中文粘贴、方向键不误关弹层、resize 后退出，以及 stty 属性与光标/鼠标/粘贴/备用屏幕恢复。

见 [M0-17a 证据](evidence/m0-17a-20260916/README.md)。这是终端输入子片，未覆盖断线续读、草稿持久恢复或 Windows/Ubuntu；完整 CAP-17 及独立验收继续留空。100ms 是 Escape 与控制序列的消歧窗口，不承诺任意网络延迟下都能合并分段序列。
