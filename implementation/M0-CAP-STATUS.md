...
| **04 标题、正文、字段编辑** · 部分实现 | 原生 `TrackerItemDetail`，适配器 `update-item`/`update-items → task.update`；`update-item-content` 路由存在 | `e` 仅提供标题/正文编辑，携带选中记录 revision | `task update --expected-revision` | `babel/tests/edit-revision-closed-loop.test.ts` 验证适配/HTTP/headless 输入及过期版本；本轮 `title-native.spec.ts` 原生中文标题保存、跨端更新与冲突不覆盖已通过。标题使用显式保存及草稿起始 revision。**下一步**：Mac 原生正文、字段保存→PTY/CLI 读回；并发编辑保草稿。单独核对 `update-item-content` 的 revision 传递与富文本语义。 |
| **05 依赖、关联、优先级** · 部分实现 | 原生字段/关系控件保留，通用更新适配存在；未确认所有关系入口均命中同一双向关系守卫 | `l` 依赖/blocks 表单→`relation.set`；未见完整任意字段/优先级编辑表单 | `relation set`、`task update` 字段 | `babel/tests/domain-lifecycle.test.ts`、`cli-lr03.test.ts`、`tui-lr03.test.ts` 覆盖关系正反向和命令。**下一步**：原生关系操作和优先级→PTY 编辑→CLI 验双向关系/revision；补循环/只读/冲突负例的实际输入。 |
...
| **10 取消、终止核对** · 部分实现 | 运行页/工具栏取消→`run.cancel`；未见完整 `run.reconcile` GUI 入口 | `c` 取消确认；核对菜单目前仅 `lost` 显示，未覆盖 `cancel_requested` | `run cancel`、`run reconcile` | `babel/tests/guards.test.ts`、`domain-lifecycle.test.ts`；本轮恢复测试证明重启不擅自把取消待确认/失联改成已停止。**下一步**：补 GUI 和 TUI 待确认核对入口，真实输入测试未确认禁止重试/归档，确认后允许新 run。 |
