...
export type TrackerIdentityRecoveryPlan =
  | { action: 'none'; reason: 'no-stranded-rows' | 'already-attempted' | 'cursor-not-ahead' }
  | { action: 'resync'; sinceSyncId: SyncId; strandedCount: number; rewindDistance: number };
 ...
 */
export function planTrackerIdentityRecovery(
  facts: StrandedIdentityFacts,
): TrackerIdentityRecoveryPlan {
  if (facts.alreadyAttempted) return { action: 'none', reason: 'already-attempted' };
  if (facts.strandedCount <= 0) return { action: 'none', reason: 'no-stranded-rows' };
  if (facts.minStrandedSyncId >= facts.localMaxSyncId) {
    return { action: 'none', reason: 'cursor-not-ahead' };
  }

  // Ask from one below the oldest stranded row so that row itself is included
  // -- the server filters with `syncId > sinceSyncId`.
  const sinceSyncId = Math.max(0, Number(facts.minStrandedSyncId) - 1);
  return {
    action: 'resync',
    sinceSyncId,
    strandedCount: facts.strandedCount,
    rewindDistance: Number(facts.localMaxSyncId) - sinceSyncId,
  };
}
