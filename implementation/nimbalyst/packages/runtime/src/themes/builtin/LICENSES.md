# Third-Party Theme Licenses

This directory contains themes from third-party creators. Below are the licensing details for each.

## Solarized Dark & Solarized Light

**Author**: Ethan Schoonover
**License**: MIT License
**Copyright**: © 2011 Ethan Schoonover
**Homepage**: https://ethanschoonover.com/solarized/

See `solarized-dark/LICENSE` and `solarized-light/LICENSE` for the full license text.

## Monokai

**Author**: Wimer Hazenberg
**License**: MIT License
**Copyright**: © 2006 Wimer Hazenberg
**Homepage**: https://monokai.pro/

See `monokai/LICENSE` for the full license text.

## Other Themes

The following themes are created by Nimbalyst and are licensed under the same license as the Nimbalyst application:

- **Light**: Default light theme
- **Dark**: Default dark theme
- **Crystal Dark**: GitHub-inspired dark theme
