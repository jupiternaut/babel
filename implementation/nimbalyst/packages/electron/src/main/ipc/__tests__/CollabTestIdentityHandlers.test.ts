...
describe("registerCollabTestIdentityHandlers", () => {
  ...
  it("hands the renderer a pre-authorized URL query instead of rewriting sockets", async () => {
    ...
    expect(removeHandlerMock.mock.calls.map(([channel]) => channel)).toEqual(
      AUTH_CHANNELS
    );
    expect(safeHandleMock.mock.calls.map(([channel]) => channel)).toEqual(
      AUTH_CHANNELS
    );
    ...
    expect(
      await handlers.get("team:find-for-workspace")!(event, "/tmp/ws")
    ).toMatchObject({
      success: true,
      complete: true,
      team: { orgId: "e2e-org", name: "e2e-org" },
    });
    ...
    const index = await handlers.get("document-sync:resolve-index-config")!(
      event,
      {
        workspacePath: "/tmp/ws",
      }
    ...
    expect(
      await handlers.get("document-sync:get-jwt")!(event, { orgId: "e2e-org" })
    ).toEqual({ success: true, jwt: expect.any(String) });
    expect(
      await handlers.get("document-sync:get-jwt")!(event, {
        orgId: "other-org",
      })
