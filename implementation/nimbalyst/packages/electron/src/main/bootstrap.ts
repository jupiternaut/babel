...
// ESM hoists static imports, so `import './index.js'` would construct
// logger/app-settings stores before `app.setPath('userData')`. Load the
// rest of main only after the isolated profile path is applied.
// electron-vite emits this file as CJS, which cannot use top-level await.
