...
import { projectOpsSurface, type OfflineSurfacesInput } from './babelSurfaceProjection';

export type BabelOpsSettingsProps = OfflineSurfacesInput;

/**
 * Read-only ops / chat-inbox notes. Demo or unconnected only.
 * Does not probe user services, restart processes, or claim production health.
 */
export const BabelOpsSettings: React.FC<BabelOpsSettingsProps> = (props) => {
  const view = projectOpsSurface(props);

  return (
    <section
      className="flex min-h-0 flex-col border-b border-nim px-1.5 py-2"
      data-testid="babel-ops-settings"
      data-access-label={view.accessLabel}
      data-real-ops={view.realOps ? 'true' : 'false'}
      data-real-health={view.realHealth ? 'true' : 'false'}
      data-restart-executed={view.restartExecuted ? 'true' : 'false'}
    >
      <h3 className="mb-1 px-2 text-[10px] font-semibold uppercase tracking-wider text-nim-faint">
        运维
      </h3>

      <p className="px-2 text-[12px] text-nim" data-testid="babel-ops-access">
        {view.demoLabel}
      ...
      <dl className="mt-2 flex flex-col gap-1 px-2 text-[11px] text-nim-muted">
        <div className="flex min-h-8 flex-wrap items-baseline justify-between gap-2">
          <dt>健康探测</dt>
          <dd className="text-nim" data-testid="babel-ops-health">
            {view.healthLabel}
          </dd>
        </div>
        <div className="flex min-h-8 flex-wrap items-baseline justify-between gap-2">
          <dt>待确认草稿</dt>
          <dd className="text-nim">{view.draftCount}</dd>
        </div>
        <div className="flex min-h-8 flex-wrap items-baseline justify-between gap-2">
          <dt>聊天厂商</dt>
          <dd className="text-nim">未选定</dd>
        </div>
      </dl>
      <p className="px-2 pt-0.5 text-[11px] text-nim-faint">{view.healthNote}</p>
      ...
      <ul className="flex flex-col gap-1">
        {view.layers.map((layer) => (
          <li
            ...
            data-testid={`babel-ops-layer-${layer.id}`}
          >
      ...
      {view.errors.length > 0 ? (
        <ul className="mt-2 flex flex-col gap-1 px-2" data-testid="babel-ops-errors" role="alert">
