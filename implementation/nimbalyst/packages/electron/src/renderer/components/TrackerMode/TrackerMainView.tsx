...
function trackerIdFromCommandResult(result: unknown): string | undefined {
  if (!result || typeof result !== 'object') return undefined;
  const id = (result as { trackerId?: unknown }).trackerId;
...
interface TrackerMainViewProps {
  ...
  displaySurface?: 'native' | 'execution';
  onDisplaySurfaceChange?: (surface: 'native' | 'execution') => void;
  babelListedIds?: Set<string> | null;
  babelScopeUnavailable?: boolean;
}
...
export const TrackerMainView: React.FC<TrackerMainViewProps> = ({
  ...
  onUpdateSavedView,
  onExitSavedView,
  displaySurface = 'native',
  onDisplaySurfaceChange,
  babelListedIds = null,
  babelScopeUnavailable = false,
}) => {
  ...
  const inboxFilteredItems = useMemo(() => {
    const rows = inboxScope === 'global' ? globalViewFilteredItems : viewFilteredItems;
    if (!scopedListedIds) return rows;
    return rows.filter((item) => scopedListedIds.has(item.id));
  }, [scopedListedIds, globalViewFilteredItems, inboxScope, viewFilteredItems]);

  const scopedViewFilteredItems = useMemo(() => {
    if (!scopedListedIds) return viewFilteredItems;
  ...
  const viewItemsWithPersonalFields = useMemo(() => {
    ...
    if (!scopedListedIds) return mapped;
    return mapped.filter((item) => scopedListedIds.has(item.id));
  }, [scopedListedIds, leverageOrderActive, readinessByItemId, viewFilteredItems, viewedAtByItemId]);
  ...
  const handleCloseDetail = useCallback(() => {
    ...
    queueMicrotask(() => {
      if (!id) return;
      const card = document.querySelector(`[data-item-id="${CSS.escape(id)}"]`) as HTMLElement | null;
      card?.focus();
    });
  ...
  const handleArchiveItem = useCallback(async (itemId: string, archive: boolean) => {
    try {
      if (babelDemoSource) {
        const caps = await babelDemoSource.getCapabilities(itemId);
        const action = archive ? caps.actions['task.archive'] : caps.actions['task.restore'];
        if (action && !action.allowed) {
          errorNotificationService.showError(archive ? '无法归档' : '无法恢复', action.reason ?? '当前状态不允许该操作');
          return;
        }
        await babelDemoSource.command({ type: 'archive-item', itemId, archive });
        return;
      }
      if (isBabelDemoWorkspace(workspacePath)) {
        errorNotificationService.showError(
          archive ? '无法归档' : '无法恢复',
          '演示工作区必须通过 Babel 命令归档或恢复，不能改写宿主 Tracker。',
        );
        return;
      }
      ...
      const formatted = formatBabelHostError(error);
      errorNotificationService.showError(archive ? '无法归档' : '无法恢复', formatted.message);
      console.error('[TrackerMainView] Failed to archive item:', error);
    }
  }, [babelDemoSource, workspacePath]);

  const handleDeleteItem = useCallback(async (itemId: string) => {
    if (babelDemoSource) {
      const denied = unimplementedCommandError('delete-item');
      errorNotificationService.showError('无法删除', denied.message);
      return;
    }
    try {
  ...
  }, [babelDemoSource, selectedItemId, setModeLayout]);
  ...
  const handleArchiveItems = useCallback(async (itemIds: string[], archive: boolean) => {
    for (const itemId of itemIds) {
      await handleArchiveItem(itemId, archive);
    }
  }, [handleArchiveItem]);
  ...
  const handleQuickAddSubmit = useCallback(async (title: string, priority: string) => {
    ...
    try {
      ...
      if (babelDemoSource) {
        const created = await babelDemoSource.command({
          type: 'create-item',
          item: {
            ...
            status: built.payload.status ?? 'to-do',
          },
        });
        const createdId = trackerIdFromCommandResult(created.result);
      ...
      if (isBabelDemoWorkspace(workspacePath)) {
        errorNotificationService.showError(
          '无法新建',
          '演示工作区必须通过 Babel 命令新建，不能改写宿主 Tracker。',
        );
        return;
      }
      ...
      const formatted = formatBabelHostError(error);
      errorNotificationService.showError('无法新建', formatted.message);
      console.error('[TrackerMainView] Failed to create tracker item:', error);
    }
  }, [workspacePath, quickAddType, setModeLayout, babelDemoSource]);
  ...
  const detailNode = detailItemId ? (
    <BabelExecutionShell
      enabled={displaySurface === 'execution'}
      trackerId={detailItemId}
      dataSource={babelDemoSource}
      onClose={handleCloseDetail}
      hiddenByScope={Boolean(
        selectedItemId
        && scopedListedIds
        && !scopedListedIds.has(selectedItemId),
      ) || babelScopeUnavailable}
    >
    <TrackerItemDetail
      itemId={detailItemId}
      workspacePath={workspacePath}
  ...
  const itemChrome = (
    <>
      ...
      <div className="tracker-toolbar flex items-center gap-2 px-3 py-2 border-b border-nim bg-nim shrink-0">
        ...
        <div className="inline-flex h-7 overflow-hidden rounded border border-nim" role="group" aria-label="显示表面">
          <button
            type="button"
            className={`px-2 text-[11px] ${displaySurface === 'native' ? 'bg-nim-tertiary text-nim' : 'text-nim-muted'}`}
            aria-pressed={displaySurface === 'native'}
            onClick={() => onDisplaySurfaceChange?.('native')}
            data-testid="tracker-surface-native"
          >
            原生
          </button>
          <button
            type="button"
            className={`px-2 text-[11px] ${displaySurface === 'execution' ? 'bg-nim-tertiary text-nim' : 'text-nim-muted'}`}
            aria-pressed={displaySurface === 'execution'}
            onClick={() => onDisplaySurfaceChange?.('execution')}
            data-testid="tracker-surface-execution"
          >
            执行视图
          </button>
        </div>
    ...
    <>
      {displaySurface === 'execution' ? (
            ...
            <KanbanBoard
              ...
              onItemSelect={handleItemSelect}
              selectedItemId={selectedItemId}
              overrideItems={scopedViewFilteredItems}
              onArchiveItems={handleArchiveItems}
              onDeleteItems={handleDeleteItems}
              onCopyDeepLink={teamOrgId ? handleCopyDeepLink : undefined}
              onOpenDocument={handleOpenItemAsDocument}
