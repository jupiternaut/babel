...
import { projectPdfSurface, type OfflineSurfacesInput } from './babelSurfaceProjection';

export type BabelPdfSettingsProps = OfflineSurfacesInput;

/**
 * Read-only PDF anchor / bilingual notes. Demo or unconnected only.
 * Does not open a reader, call a translator, or claim a live translation.
 */
export const BabelPdfSettings: React.FC<BabelPdfSettingsProps> = (props) => {
  const view = projectPdfSurface(props);

  return (
    <section
      className="flex min-h-0 flex-col border-b border-nim px-1.5 py-2"
      data-testid="babel-pdf-settings"
      data-access-label={view.accessLabel}
      data-real-translation={view.realTranslation ? 'true' : 'false'}
      data-live-reader={view.liveReader ? 'true' : 'false'}
    >
      <h3 className="mb-1 px-2 text-[10px] font-semibold uppercase tracking-wider text-nim-faint">
        PDF
      </h3>

      <p className="px-2 text-[12px] text-nim" data-testid="babel-pdf-access">
        {view.demoLabel}
      ...
      <dl className="mt-2 flex flex-col gap-1 px-2 text-[11px] text-nim-muted">
        <div className="flex min-h-8 flex-wrap items-baseline justify-between gap-2">
          <dt>对译</dt>
          <dd className="text-nim" data-testid="babel-pdf-translation">
            {view.translationLabel}
          </dd>
        </div>
        <div className="flex min-h-8 flex-wrap items-baseline justify-between gap-2">
          <dt>文档</dt>
          <dd className="min-w-0 break-words text-nim">
            {view.documentId || '未打开'}
          </dd>
        </div>
        <div className="flex min-h-8 flex-wrap items-baseline justify-between gap-2">
          <dt>测试批注</dt>
          <dd className="text-nim">{view.annotationCount}</dd>
        </div>
      </dl>
      <p className="px-2 pt-0.5 text-[11px] text-nim-faint">{view.translationNote}</p>
      ...
      <ul className="flex flex-col gap-1">
        {view.layers.map((layer) => (
          <li
            ...
            data-testid={`babel-pdf-layer-${layer.id}`}
          >
      ...
      {view.errors.length > 0 ? (
        <ul className="mt-2 flex flex-col gap-1 px-2" data-testid="babel-pdf-errors" role="alert">
