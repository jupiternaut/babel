  ...
  try {
    ...
    if (savedModeLayout && typeof savedModeLayout === 'object') {
      ...
      const newLayout: TrackerModeLayout = {
        ...
        displaySurface: savedModeLayout.displaySurface === 'execution' ? 'execution' : 'native',
      };
...
export interface TrackerModeLayout {
  ...
  /**
   * Babel execution surface. Native list/board/table stay on `native`.
   * Switching this flag must not remount the host editor or start a run.
   */
...
export const trackerHostDataSourceAtom = atom<TrackerDataSource | null>(null);
const trackerDataSourceAtom = trackerHostDataSourceAtom;
