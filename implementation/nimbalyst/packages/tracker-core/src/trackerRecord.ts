...
import type {
  LegacyTrackerItem as TrackerItem,
  TrackerActivity,
  TrackerComment,
  TrackerIdentity,
  TrackerItemSource,
...
import { fromDbBoolean } from './dbBoolean.js';
import {
  derivePlanStatusSignals,
  type TrackerDerivedSignal,
} from './planStatusIntegrity.js';

export type { TrackerDerivedSignal } from './planStatusIntegrity.js';

// Re-exported so hosts reading tracker rows share one boolean coercion.
...
/**
 * Stored tracker rows use `null` for a comment that has never been edited,
 * while the canonical record and sync protocol represent that state by an
 * absent optional timestamp. Normalize at the read boundary so persisted
 * legacy values do not leak into consumers.
 */
function normalizeTrackerComments(value: unknown): TrackerComment[] | undefined {
  if (!Array.isArray(value)) return undefined;

  return value.map((comment) => {
    if (!comment || typeof comment !== 'object' || Array.isArray(comment)) {
      return comment as TrackerComment;
    }
    const storedComment = comment as Record<string, unknown>;
    if (storedComment.updatedAt != null) return storedComment as unknown as TrackerComment;

    const { updatedAt: _storedNull, ...normalizedComment } = storedComment;
    return normalizedComment as unknown as TrackerComment;
  });
}
...
export function trackerItemToRecord(item: TrackerItem): TrackerRecord {
  ...
  // NEVER fabricate `new Date()` for a missing created/updated. This
  ...
  if (item.customFields) {
    for (const key of SYSTEM_KEYS) {
      if (key in item.customFields && (record.system as any)[key] === undefined) {
        (record.system as any)[key] = key === 'comments'
          ? normalizeTrackerComments(item.customFields[key])
...
export function dbRowToRecord(row: any): TrackerRecord {
  ...
  const record: TrackerRecord = {
    ...
    system: {
      ...
      comments: normalizeTrackerComments(systemValue('comments')),
