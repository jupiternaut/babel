...
export class CliHttp {
  ...
  constructor(options: CliHttpOptions = {}) {
    ...
    this.actor = options.actor ?? { ...DEMO_ACTOR, kind: "cli" };
  ...
  watchEvents(projectId: string, cursor: string | undefined, onEvent: (event: BabelEvent) => void, onError?: (error: Error) => void): { close: () => void } {
    ...
    req.on("response", (res) => {
      if (res.statusCode && res.statusCode >= 400) {
        ...
        res.on("end", () => {
          ...
          onError?.(new BabelError("UNAVAILABLE", `事件流返回 HTTP ${res.statusCode}`, { status: res.statusCode }, true));
        });
        return;
      }
      ...
      const req = transport(url, {
        ...
        res.on("end", () => {
          ...
          try {
            ...
            reject(new BabelError("UNAVAILABLE", "响应不是合法 JSON", { cause: String(error), text }, true));
          }
        });
      });
