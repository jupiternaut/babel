...
export const COLLAB_DOCUMENT_DRAG_TYPE = 'application/x-nimbalyst-collab-document';
 ...
 */
export function parseCollabDocumentDrag(
  raw: string | null | undefined,
): CollabDocumentDragPayload | null {
  if (!raw) return null;
  try {
    const parsed: unknown = JSON.parse(raw);
    if (typeof parsed !== 'object' || parsed === null) return null;
    const { orgId, documentId, title } = parsed as Record<string, unknown>;
    if (typeof orgId !== 'string' || !orgId) return null;
    if (typeof documentId !== 'string' || !documentId) return null;
    return {
      orgId,
      documentId,
      title: typeof title === 'string' ? title : documentId,
    };
  } catch {
    return null;
  }
