...
import type { TrackerViewMode } from '@nimbalyst/collab-client/trackers';
 ...
 */
export const VIEW_MODE_FALLBACK: TrackerViewMode = 'list';
...
export function resolveViewMode(
  requested: TrackerViewMode,
  capabilities: ViewModeCapabilities,
): ResolvedViewMode {
  return capabilities.renderableViewModes.has(requested)
    ? { mode: requested, substituted: false }
    : { mode: VIEW_MODE_FALLBACK, substituted: true };
}
